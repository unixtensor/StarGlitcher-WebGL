...
import { rE_Root_PROPERTIES, rE_CameraControls, rE_Root } from './rE_Root'
...
export class rE_KeyMap {
	...
	update(deltaTime = 1) {
		if (InputEvent.w) {
			const FORWARD_Direction = new Vector3(0,0,-(Math.abs(Math.min(rE_Root_PROPERTIES.WalkSpeed,100)))).applyQuaternion(this.CAMERA.quaternion)
		...
		if (InputEvent.a) {
			const RIGHT_Direction = new Vector3(-(Math.abs(Math.min(rE_Root_PROPERTIES.WalkSpeed,100))),0).applyQuaternion(this.CAMERA.quaternion)
		...
		if (InputEvent.d) {
			const RIGHT_Direction = new Vector3(-(Math.abs(Math.min(rE_Root_PROPERTIES.WalkSpeed,100))),0,0).applyQuaternion(this.CAMERA.quaternion)
			this.ROOT.position.x -= RIGHT_Direction.x*deltaTime
			this.ROOT.position.z -= RIGHT_Direction.z*deltaTime
			if (!Control_IgnoreY)
				this.ROOT.position.y -= RIGHT_Direction.y*deltaTime
